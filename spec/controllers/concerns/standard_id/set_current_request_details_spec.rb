...
RSpec.describe StandardId::SetCurrentRequestDetails do
  ...
  # `stub_const` assigns the anonymous subclass to the constant, which gives it
  # the name "Current" — and `CurrentAttributes` memoizes its per-execution
  # instance under a key derived from that name. So the stub's instance lands on
  # the same key as the app's real `Current`.
  #
  # `Current.reset` only blanks that instance's values; it leaves the anonymous
  # instance memoized. Once `stub_const` restores the real class, its
  # class-level readers (generated by `attribute` as delegators to `instance`)
  # keep resolving to the stub's instance, which declares only the three
  # attributes above — so e.g. `Current.account` raised NoMethodError for every
  # later example in the process. `clear_all` drops the memo as well as the
  # values, so the real class re-instantiates cleanly.
