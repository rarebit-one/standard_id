...
RSpec.describe StandardId::Events::Subscribers::AccountLockingSubscriber do
  ...
  describe ".subscribe_to" do
    it "subscribes to ACCOUNT_LOCKED event" do
      expect(described_class.subscribed_events).to include(StandardId::Events::ACCOUNT_LOCKED)
    end
  end
  ...
  describe "#call" do
    let(:event) do
      StandardId::Events::Event.new(
        name: "standard_id.#{StandardId::Events::ACCOUNT_LOCKED}",
        payload: { account: account, locked_by: admin, reason: "Policy violation" },
    ...
    context "when revoke_sessions_on_lock is enabled (default)" do
      before do
        allow(StandardId.config.account_locking).to receive(:revoke_sessions_on_lock).and_return(true)
      end
      ...
      it "emits SESSION_REVOKED event for each session" do
        ...
        events_received.each do |e|
          expect(e[:reason]).to eq("account_locked")
        end
      end
    end
  ...
  describe "integration with Events.publish" do
    ...
    it "revokes sessions when ACCOUNT_LOCKED is published" do
      active_sessions = account.sessions.active
      expect(active_sessions.count).to be > 0
