module StandardId
  ...
  # all Rails controllers) and to include `StandardId::WebAuthentication` for
  # the session_manager dependency (needed by verify to resolve accounts).
  #
  ...
  #
  #     def create
  #       generate_passwordless_otp(email: params[:email])
  #       redirect_to verify_path
  #     end
  #
  #     def verify
  #       result = verify_passwordless_otp(email: params[:email], code: params[:code])
  #       if result.success?
  ...
  module PasswordlessFlow
    ...
    # @raise [StandardId::InvalidRequestError] when the email format is invalid
    #   or the connection type is unsupported
    def generate_passwordless_otp(email:, connection: "email")
    ...
    #
    # @param email [String] the identifier value (email or phone number)
    ...
    def verify_passwordless_otp(email:, code:, connection: "email", allow_registration: true)
