...
- **Open-redirect validation extended to password, signup, and logout** — `safe_destination?` and `safe_post_signin_default` are now promoted to `Web::BaseController` and applied to the destination chain for password login, password signup, and logout (was previously social callback only). Cross-host URLs not in `allowed_redirect_url_prefixes` and protocol-relative URLs (`//evil.com/`) fall back to `after_authentication_url` / `root_path` instead of redirecting to an attacker-controlled target.
